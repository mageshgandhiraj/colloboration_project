...
public class FriendDaoImpl implements FriendDao {
	...
	public List<User> listOfSuggestedUsers(String email) {
		...
		SQLQuery sqlQuery = session.createSQLQuery(
				"select * from user_s180233 where email in (select email from user_s180233 where email!=? minus (select fromId from friend_s180233 where toId=? union select toId from friend_s180233 where fromId=?))");
	...
	public List<User> listOfFriends(String email) {
		...
		SQLQuery query = session.createSQLQuery("select * from user_s180233 where email in"
				+ "(select fromId from friend_s180233 where toId=? and status='A' " + " union "
